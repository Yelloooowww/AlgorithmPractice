...
void multiply(int *A,int *B,int *C,int n){
  ...
  if(n<=1){
    ...
    *C = (*A)*(*B);
    ...
    sub(B+(1*2+0)*((n*n)/4),B+(0*2+0)*((n*n)/4),&tmp1[0][0],n/2);
...
int main(){
  ...
  int C[4][4];
  // sub(&A[0][0],&B[0][0],&C[0][0],4);
  // cout<<"A:"<<A<<endl;
  // print_Maxtrix(&A[0][0],4);
  // cout<<"B:"<<endl;
  // print_Maxtrix(&B[0][0],4);
  // cout<<"C:"<<endl;
  // print_Maxtrix(&C[0][0],4);
  multiply(&A[0][0],&B[0][0],&C[0][0],4);
